odoo.define('theme_laze.brandCatDialog', function (require) {
"use strict";

const webEditor = require('web_editor.widget');
const webUtils = require('web.utils');
const options = require('web_editor.snippets.options');
const { BaseLazeQweb, FetchCategoryData, FetchBrandData, FetchBlogData } = require("theme_laze.core_mixins");
const brandCatDialog = webEditor.Dialog;
const webCore = require('web.core');
const _t = webCore._t;
const recordSelector = require('theme_laze.recordSelector')

let baseBrandCat = brandCatDialog.extend(FetchBrandData, FetchBlogData, FetchCategoryData, BaseLazeQweb,{
    ...
    _initData: function(rec, popupType){
        ...
        if (cr.opts.popupType == "Brand") {
            const BrandTemp = cr._baseLazeQweb("theme_laze.dialog_brand_list_view_init",rec);
            ...
            const CatTemp = cr._baseLazeQweb("theme_laze.dialog_categ_list_view_init",rec);
            let $mainDataDiv = cr.$el.find(".as-cat-select-list-view");
            $mainDataDiv.empty().append(CatTemp);
            $mainDataDiv.find(".as-sort-data").sortable();
        } else if (cr.opts.popupType == "Blog") {
            const BlogTemp = cr._baseLazeQweb("theme_laze.dialog_blog_list_view_init",rec);
    ...
    _openBrandDialog: function(ev) {
        ...
        let select2InitData = {
            ...
            searchType:'BrandMix',
            customTemplate:'theme_laze.as_select2_brand_dropdown'};
    ...
    _openBlogDialog: function(ev) {
        ...
        let select2InitData = {
            ...
            extraData:[],
            parentDomain:false,
            customTemplate:'theme_laze.as_select2_blog_dropdown'};
        cr.recordSelector = new recordSelector(cr,select2InitData);
        cr.recordSelector.open();
    },
