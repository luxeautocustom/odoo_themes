...
let baseProduct = productDialog.extend(FetchProductsData, BaseLazeQweb,{
    ...
    willStart: function(){
        ...
        return cr._super.apply(cr, arguments).then(function(){
            if(cr.opts.popupType == "Product" || cr.opts.popupType == "Product Offer") {
    ...
    _initData: function(popupType, rec){
        ...
        if (popupType == "Product") {
            ...
            const ProdTemp = cr._baseLazeQweb("theme_laze.dialog_product_list_view_init",clean_res);
    ...
    _onSavebtn: function(ev) {
        var cr = this;
        if (cr.src.data.selector == ".as_product_slider" || cr.src.data.selector == ".as_product_offer") {
            let prePosData = cr._prodDataFetch();
