odoo.define('theme_laze.core_mega_dialog', function (require) {
"use strict";
...
const _t = webCore._t;
const RecordSelectorDialog = require("theme_laze.recordSelector");
const { LazeTemplateGetter,
        FetchProductsData,
        FetchCategoryData,
        BaseLazeQweb } = require("theme_laze.core_mixins");

let BaseMega = megaDialog.extend(LazeTemplateGetter, FetchProductsData, FetchCategoryData, BaseLazeQweb,{
    template: 'theme_laze.core_dialog',
    xmlDependencies: megaDialog.prototype.xmlDependencies.concat([
        '/theme_laze/static/src/xml/core_dialog.xml',
    ...
    willStart:function(){
        ...
        return cr._super.apply(cr, arguments).then(function(){
            if(cr.opts.megaPopup == "product_mega_modal"){
            ...
            else if(cr.opts.megaPopup == "category_mega_modal"){
                if(cr.opts.initRecords != undefined && cr.opts.megaUi != undefined){
                    if(cr.opts.initRecords.trim() != ""){
    ...
    _initData:function (rec, megaType) {
        ...
        if(megaType == "Product"){
            ...
            const ProdTemp = cr._baseLazeQweb("theme_laze.dialog_product_list_view",clean_res);
            ...
            var opts = cr.opts;
            const ProdTemp =  cr._baseLazeQweb("theme_laze.dialog_cat_list_view",rec);
    ...
    _openProductSelectorDialog: function(ev){
        ...
        let select2InitData = {
            ...
            searchType:'Product',
            customTemplate:'theme_laze.as_select2_products_dropdown'};
    ...
    _openCategorySelectorDialog:function(){
        ...
        let select2InitData = {
            ...
            parentDomain:false,
            searchType:'Category',
            customTemplate:'theme_laze.as_select2_category_dropdown' };
        cr.recordSelector = new RecordSelectorDialog(cr, select2InitData);
        cr.recordSelector.open();
    },
            ...
            cr._fetchProductRawData(prePosData[1],"product.template").then((rec) =>{
                if(rec != undefined){
                    ...
                    if(megaProdView == "list"){
                        cr._getLazeTemplate("atharva_theme_base","as_mm_product_list",data).then((res) =>{
                            cr.src.$target.parent().empty().append(res['template']);
                        });
                    }else if(megaProdView == "grid"){
                        cr._getLazeTemplate("atharva_theme_base","as_mm_product_grid",data).then((res) =>{
                            cr.src.$target.parent().empty().append(res['template']);
                        });
                    }else{
