...
class ThemeLazeBaseFeatureRoute(http.Controller):
    ...
    def _lazeDataGetter(self, dataMethod=False, extra_field=[], limit=10, domain=[], **kwargs):
        ...
        if dataMethod == "select2_product":
            ...
            return self._fetchBasicProdData(products)
    ...
    @http.route("/laze_template_getter", type="json", auth="public", website=True)
    def lazeTemplateGetters(self,**kwargs):
    ...
    def getSelect2Data(self, searchType=False, **kwargs):
        ...
        if searchType in [ "Product", "ImageHotspot", "CatProdGrid", "CategProd", "ProdOffer"]:
            ...
            records = self._lazeDataGetter(dataMethod="select2_product", domain=domain, limit=15)
            ...
            domain = expression.AND([request.website.website_domain(), searchTerm, parentDomain])
            records = self._lazeDataGetter(dataMethod="select2_category", domain=domain, limit=15)
        elif searchType == "BrandMix":
            searchTerm = [("name", "ilike", (kwargs.get("searchTerm") or ""))]
            domain = expression.AND([request.website.website_domain(), searchTerm])
            records = self._lazeDataGetter(dataMethod="select2_brand", domain=domain, limit=15)
            ...
            searchTerm = [("name", "ilike", (kwargs.get("searchTerm") or ""))]
            domain = expression.AND([request.website.website_domain(), searchTerm])
            records = self._lazeDataGetter(dataMethod="select2_blog", domain=domain, limit=15)
        return json.dumps(records)

    @http.route("/get/product/raw/data", type="json", auth="public", website=True)
    def lazeProductRawData(self, product_ids, model):
    ...
    def lazeCategoryRawData(self, category_ids):
        ''' Method to fatch product variant and product template data'''
        website_domain = request.website.website_domain()
        if category_ids:
    ...
    def lazeBrandRawData(self, brand_ids):
        if brand_ids:
    ...
    def lazeBlogRawData(self, blog_ids):
        website_domain = request.website.website_domain()
        if blog_ids:
    ...
    def lazeCategoryMegaTemplate(self, **kwargs):
