...
class LazeAuthSystem(Home):

    @http.route('/laze/login/',type='json',auth="public")
    def laze_login(self,**kwargs):
        ...
        login_template = request.env['ir.ui.view']._render_template("theme_laze.as_login",context)
        return {'template':login_template}

    @http.route('/laze/login/authenticate', type='json', auth="none")
    def laze_login_authenticate(self, **kwargs):
    ...
    @http.route('/laze/signup/authenticate',type="json",auth="public")
    def laze_signup_authenticate(self,*args, **kw):
        ''' Signup Authentication '''
        qcontext = super(LazeAuthSystem,self).get_auth_signup_qcontext()
        if 'error' not in qcontext and request.httprequest.method == 'POST':
            try:
                super(LazeAuthSystem,self).do_signup(qcontext)
